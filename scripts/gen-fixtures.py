...
<form id="requestForm" method="post" action="/app/assistancerequests/3724739">
...
<table id="assistanceItemsTable">
<tr><th></th><th>Assistance</th><th>Value</th><th>Date Provided</th><th>Pending</th></tr>
{items_html}
</table>
</body></html>
"""

ITEM = ('<tr><td></td><td>{name}</td><td>${value}</td><td>{date}</td><td>{pending}</td></tr>')
...
def main():
    ...
    (OUT / "detail_completed.html").write_text(detail_page(
        ...
        items_html=(ITEM.format(name="Second Harvest Food", value="70", date="09/05/2026", pending="No")
                    + ITEM.format(name="Gift Cards", value="90", date="09/05/2026", pending="No"))))
